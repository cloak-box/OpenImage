...
public abstract class OpenImageActivity extends BaseActivity
    implements TouchCloseLayout.OnTouchCloseListener {
  ...
  private void initPhotosViewModel() {
    ...
    photosViewModel.onAddOnSelectMediaListenerLiveData.observe(
        this,
        s -> {
          OnSelectMediaListener onSelectMediaListener =
              ImageLoadUtils.getInstance().getOnSelectMediaListener(s);
          if (onSelectMediaListener != null) {
            ...
            if (isFirstBacked && showPosition < getOpenImageBeans().size()) {
              ...
              onSelectMediaListener.onSelect(
                  getOpenImageBeans(), imageDetail.openImageUrl, showPosition);
            }
          }
          onSelectMediaListenerKeys.add(s);
        });

    photosViewModel.onRemoveOnSelectMediaListenerLiveData.observe(
        this,
        s -> {
          OnSelectMediaListener onSelectMediaListener =
              ImageLoadUtils.getInstance().getOnSelectMediaListener(s);
    ...
    photosViewModel.choseItemLiveData.observe(
        this,
        data -> {
          viewPager.setCurrentItem(data.second, true);
        });
  }

  private void initTouchCloseLayout() {
    boolean disEnableTouchClose =
        getIntent().getBooleanExtra(OpenParams.DISABLE_TOUCH_CLOSE, false);
    ...
    if (touchCloseOrientation != null) {
      ...
      rootView.setOrientation(
          orientation == OpenImageOrientation.VERTICAL
              ? OpenImageOrientation.HORIZONTAL
              : OpenImageOrientation.VERTICAL);
    }
  ...
  private void initViewPager2() {
    ...
    VideoFragmentCreate videoCreate =
        ImageLoadUtils.getInstance().getVideoFragmentCreate(videoFragmentCreateKey);
    ImageFragmentCreate imageCreate =
        ImageLoadUtils.getInstance().getImageFragmentCreate(imageFragmentCreateKey);
    if (videoCreate == null) {
    ...
    openImageAdapter =
        new OpenImageFragmentStateAdapter(this, viewPager) {
          @NonNull
          @Override
              ...
              if (videoFragmentCreate != null) {
                ...
                if (fragment == null) {
                  throw new IllegalArgumentException(
                      videoFragmentCreate.getClass().getName() + "请重写createVideoFragment");
                }
              } else {
                throw new IllegalArgumentException(
                    "请设置视频播放器fragment --> https://github.com/FlyJingFish/OpenImage/wiki");
              }
        ...
        new ViewPager2.OnPageChangeCallback() {
          ...
          public void onPageSelected(int position) {
            ...
            if (onSelectMediaListener != null) {
              onSelectMediaListener.onSelect(
                  getOpenImageBeans(),
                  getOpenImageBeans().get(showPosition).openImageUrl,
                  showPosition);
            }
            for (OnSelectMediaListener selectMediaListener : onSelectMediaListeners) {
              selectMediaListener.onSelect(
                  getOpenImageBeans(),
                  getOpenImageBeans().get(showPosition).openImageUrl,
                  showPosition);
            }
  ...
  protected void initMoreView() {
    if (moreViewKey != null) {
      List<MoreViewOption> viewOptions =
          ImageLoadUtils.getInstance().getMoreViewOption(moreViewKey);
      for (MoreViewOption moreViewOption : viewOptions) {
        ...
        if (view != null) {
          ...
          OnLoadViewFinishListener onLoadViewFinishListener =
              moreViewOption.getOnLoadViewFinishListener();
  ...
  protected void showMoreView() {
    ...
    if (moreViewOptions.size() > 0) {
      for (MoreViewOption moreViewOption : moreViewOptions) {
        ...
        if (mediaType == MediaType.IMAGE
            && (showType == MoreViewShowType.IMAGE || showType == MoreViewShowType.BOTH)) {
          moreViewOption.getView().setVisibility(View.VISIBLE);
        } else if (mediaType == MediaType.VIDEO
            && (showType == MoreViewShowType.VIDEO || showType == MoreViewShowType.BOTH)) {
          moreViewOption.getView().setVisibility(View.VISIBLE);
        } else {
    ...
    if (upLayerFragment != null
        && (upperView = upLayerFragment.getView()) != null
        && upperLayerOption != null
        && upperLayerOption.isTouchingHide()) {
    ...
    if (downloadImageView != null) {
      if ((mediaType == MediaType.IMAGE
              && (downloadShowType == MoreViewShowType.IMAGE
                  || downloadShowType == MoreViewShowType.BOTH))
          || (mediaType == MediaType.VIDEO
              && (downloadShowType == MoreViewShowType.VIDEO
                  || downloadShowType == MoreViewShowType.BOTH))) {
    ...
    if (closeImageView != null) {
      if ((mediaType == MediaType.IMAGE
              && (closeShowType == MoreViewShowType.IMAGE
                  || closeShowType == MoreViewShowType.BOTH))
          || (mediaType == MediaType.VIDEO
              && (closeShowType == MoreViewShowType.VIDEO
                  || closeShowType == MoreViewShowType.BOTH))) {
        ...
        () -> {
          if (indicatorType == INDICATOR_IMAGE) { // 图片样式
            ...
            if (indicatorTextBinding != null) {
              indicatorTextBinding.tvShowPos.setText(
                  String.format(textFormat, showPosition + 1, total));
            }
          }
        });
  }
  ...
  private void initStyleConfig() {
    ...
    if (themeRes != 0) {
      ...
      fontStyle =
          FontStyle.getStyle(
              AttrsUtils.getTypeValueInt(this, themeRes, R.attr.openImage_statusBar_fontStyle));
      ...
      orientation =
          OpenImageOrientation.getOrientation(
              AttrsUtils.getTypeValueInt(this, themeRes, R.attr.openImage_viewPager_orientation));
      touchCloseOrientation =
          OpenImageOrientation.getOrientation(
              AttrsUtils.getTypeValueInt(
                  this, themeRes, R.attr.openImage_touchClose_orientation, -1));
      int pageMargin =
          (int)
              AttrsUtils.getTypeValueDimension(
                  this, themeRes, R.attr.openImage_viewPager_pageMargin, -1);
      if (pageMargin >= 0) {
        compositePageTransformer.addTransformer(new MarginPageTransformer(pageMargin));
      } else {
        compositePageTransformer.addTransformer(
            new MarginPageTransformer((int) ScreenUtils.dp2px(this, 10)));
      }
      indicatorTouchingHide =
          AttrsUtils.getTypeValueBoolean(
              this, themeRes, R.attr.openImage_indicator_touchingHide, true);
      downloadToast =
          AttrsUtils.getTypeValueBoolean(this, themeRes, R.attr.openImage_download_toast, true);
      startToast =
          (String)
              AttrsUtils.getTypeValueText(this, themeRes, R.attr.openImage_download_startToast);
      successToast =
          (String)
              AttrsUtils.getTypeValueText(this, themeRes, R.attr.openImage_download_successToast);
      errorToast =
          (String)
              AttrsUtils.getTypeValueText(this, themeRes, R.attr.openImage_download_errorToast);
      if (startToast == null) {
      ...
      requestWriteExternalStoragePermissionsFail =
          (String)
              AttrsUtils.getTypeValueText(
                  this, themeRes, R.attr.openImage_requestWriteExternalStoragePermissionsFail);
    } else {
    ...
    List<ViewPager2.PageTransformer> pageTransformers =
        ImageLoadUtils.getInstance().getPageTransformers(pageTransformersKey);
    ...
    if (leftRightPadding > 0) {
      ...
      if (recyclerView instanceof RecyclerView) {
        recyclerView.setPadding(
            (int) ScreenUtils.dp2px(this, leftRightPadding),
            0,
            (int) ScreenUtils.dp2px(this, leftRightPadding),
            0);
    ...
    if (requestWriteExternalStoragePermissionsFail == null) {
      requestWriteExternalStoragePermissionsFail =
          getString(R.string.request_WRITE_EXTERNAL_STORAGE_permissions_fail);
    }
  }
    ...
    if (themeRes != 0) {
      if (indicatorType < 2 && getOpenImageBeans().size() > 1) {
        if (indicatorType == INDICATOR_IMAGE) { // 图片样式
          float interval =
              AttrsUtils.getTypeValueDimension(
                  this, themeRes, R.attr.openImage_indicator_image_interval, -1);
          int imageRes =
              AttrsUtils.getTypeValueResourceId(
                  this, themeRes, R.attr.openImage_indicator_imageRes);
          if (interval == -1) {
          ...
          OpenImageOrientation realOrientation =
              OpenImageOrientation.getOrientation(
                  AttrsUtils.getTypeValueInt(
                      this, themeRes, R.attr.openImage_indicator_image_orientation));
          imageIndicatorLayoutManager =
              new LinearLayoutManager(
                  this,
                  realOrientation == OpenImageOrientation.HORIZONTAL
                      ? LinearLayoutManager.HORIZONTAL
                      : LinearLayoutManager.VERTICAL,
                  false);
          recyclerView.setLayoutManager(imageIndicatorLayoutManager);
          imageIndicatorAdapter =
              new ImageIndicatorAdapter(
                  getOpenImageBeans().size(), interval, imageRes, realOrientation);
          recyclerView.setAdapter(imageIndicatorAdapter);
          indicatorView = recyclerView;
        } else {
          int textColor =
              AttrsUtils.getTypeValueColor(
                  this, themeRes, R.attr.openImage_indicator_textColor, Color.WHITE);
          float textSize =
              AttrsUtils.getTypeValueDimension(this, themeRes, R.attr.openImage_indicator_textSize);
          indicatorTextBinding =
              OpenImageIndicatorTextBinding.inflate(getLayoutInflater(), rootView, true);
          FrameLayout.LayoutParams layoutParams =
              (FrameLayout.LayoutParams) indicatorTextBinding.tvShowPos.getLayoutParams();
          setIndicatorLayoutParams(layoutParams, themeRes);
          ...
          CharSequence strFormat =
              AttrsUtils.getTypeValueText(this, themeRes, R.attr.openImage_indicator_textFormat);
    ...
    if (closeShow) {
      ...
      if (closeParams == null || closeParams.getCloseLayoutParams() == null) {
        downloadLayoutParams =
            new FrameLayout.LayoutParams(
                (int) ScreenUtils.dp2px(this, 24), (int) ScreenUtils.dp2px(this, 24));
  ...
  private void initDownloadView() {
    ...
    if (downloadShow) {
      DownloadMediaHelper downloadMediaHelper =
          OpenImageConfig.getInstance().getDownloadMediaHelper();
      if (downloadMediaHelper == null) {
        if (ImageLoadUtils.getInstance().isApkInDebug()) {
          throw new IllegalArgumentException(
              "DownloadMediaHelper 不可为null 请调用 OpenImageConfig 的 setDownloadMediaHelper 来设置");
        }
        return;
      }
      ...
      if (!TextUtils.isEmpty(downloadParamsKey)) {
        ...
        if (downloadParams != null
            && (downloadShowType = downloadParams.getMoreViewShowType()) == null) {
          downloadShowType = MoreViewShowType.BOTH;
        }
      }
      FrameLayout.LayoutParams downloadLayoutParams;
  ...
  protected void checkPermissionAndDownload() {
    OnPermissionsInterceptListener onPermissionsInterceptListener =
        ImageLoadUtils.getInstance().getPermissionsInterceptListener(onPermissionKey);
  ...
  protected void downloadMedia() {
    ...
    NetworkHelper.INSTANCE.download(
        this,
        this,
        openImageDetail,
        new OnDownloadMediaListener() {
  ...
  private void setIndicatorLayoutParams(FrameLayout.LayoutParams layoutParams, int themeRes) {
    ...
    int topMargin =
        (int)
            AttrsUtils.getTypeValueDimension(this, themeRes, R.attr.openImage_indicator_marginTop);
    int bottomMargin =
        (int)
            AttrsUtils.getTypeValueDimension(
                this, themeRes, R.attr.openImage_indicator_marginBottom);
    int leftMargin =
        (int)
            AttrsUtils.getTypeValueDimension(this, themeRes, R.attr.openImage_indicator_marginLeft);
    int rightMargin =
        (int)
            AttrsUtils.getTypeValueDimension(
                this, themeRes, R.attr.openImage_indicator_marginRight);
    int startMargin =
        (int)
            AttrsUtils.getTypeValueDimension(
                this, themeRes, R.attr.openImage_indicator_marginStart);
    int endMargin =
        (int)
            AttrsUtils.getTypeValueDimension(this, themeRes, R.attr.openImage_indicator_marginEnd);
    ...
    if (gravity == 0) {
      if (orientation == OpenImageOrientation.VERTICAL) {
        ...
        layoutParams.bottomMargin =
            bottomMargin == 0 ? (int) ScreenUtils.dp2px(this, 14) : bottomMargin;
      }
    } else {
  ...
  protected void onShareTransitionEnd() {
    ...
    mHandler.post(
        () -> {
          if (upperLayerOption != null
              && OpenImageActivity.this.getLifecycle().getCurrentState()
                  != Lifecycle.State.DESTROYED) {
            UpperLayerFragmentCreate upperLayerCreate =
                upperLayerOption.getUpperLayerFragmentCreate();
            ...
            if (upLayerFragment != null) {
              ...
              transaction
                  .replace(R.id.open_image_upper_layer_container, upLayerFragment)
  ...
  private void setExitView() {
    ...
    if (shareView != null) {
      ...
      if ((shareView instanceof PhotoView)
          && (((PhotoView) shareView).getSrcScaleType() == ShapeImageView.ShapeScaleType.CENTER
              || ((PhotoView) shareView).getSrcScaleType()
                  == ShapeImageView.ShapeScaleType.CENTER_INSIDE)
          && exitView != null
          && (drawable = exitView.getDrawable()) != null) {
        ((PhotoView) shareView)
            ...
            public void onMapSharedElements(List<String> names, Map<String, View> sharedElements) {
              ...
              if ((exitView instanceof ShapeImageView) && (shareView instanceof PhotoView)) {
                ShapeImageView.ShapeScaleType shapeScaleType =
                    ((ShapeImageView) exitView).getShapeScaleType();
                ((PhotoView) shareView).setSrcScaleType(shapeScaleType);
                if (shapeScaleType == ShapeImageView.ShapeScaleType.AUTO_START_CENTER_CROP
                    || srcScaleType == ShapeImageView.ShapeScaleType.AUTO_END_CENTER_CROP) {
                  ((PhotoView) shareView)
                      .setAutoCropHeightWidthRatio(
                          ((ShapeImageView) exitView).getAutoCropHeightWidthRatio());
                }
                if (((ShapeImageView) exitView).getShapeType() == ShapeImageView.ShapeType.OVAL
                  ...
                  ((PhotoView) shareView)
                      .setRadius(
                          (int)
                              ((int) ((ShapeImageView) exitView).getLeftTopRadius()
                                  / touchCloseScale),
                          (int)
                              ((int) ((ShapeImageView) exitView).getRightTopRadius()
                                  / touchCloseScale),
                          (int)
                              ((int) ((ShapeImageView) exitView).getRightBottomRadius()
                                  / touchCloseScale),
                          (int)
                              ((int) ((ShapeImageView) exitView).getLeftBottomRadius()
                                  / touchCloseScale));
                  ((PhotoView) shareView)
                      .setRelativeRadius(
                          (int)
                              ((int) ((ShapeImageView) exitView).getStartTopRadius()
                                  / touchCloseScale),
                          (int)
                              ((int) ((ShapeImageView) exitView).getEndTopRadius()
                                  / touchCloseScale),
                          (int)
                              ((int) ((ShapeImageView) exitView).getEndBottomRadius()
                                  / touchCloseScale),
                          (int)
                              ((int) ((ShapeImageView) exitView).getStartBottomRadius()
                                  / touchCloseScale));
                }
                ...
                if (imageShapeParams.shapeType == ImageShapeType.OVAL) {
                  ...
                  ((PhotoView) shareView)
                      .setRadius(
                          (int)
                              (imageShapeParams.rectangleConnerRadius.leftTopRadius
                                  / touchCloseScale),
                          (int)
                              (imageShapeParams.rectangleConnerRadius.rightTopRadius
                                  / touchCloseScale),
                          (int)
                              (imageShapeParams.rectangleConnerRadius.rightBottomRadius
                                  / touchCloseScale),
                          (int)
                              (imageShapeParams.rectangleConnerRadius.leftBottomRadius
                                  / touchCloseScale));
                }
              }
              if (shareView instanceof PhotoView) {
  ...
  private Fragment getCurrentFragment() {
    return getSupportFragmentManager()
        .findFragmentByTag("f" + openImageAdapter.getItemId(showPosition));
  }
