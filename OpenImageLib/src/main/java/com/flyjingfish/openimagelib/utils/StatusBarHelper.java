...
public class StatusBarHelper {
  ...
  public static void translucent(Window window, @ColorInt int colorOn5x) {
    ...
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
      ...
      if (DeviceHelper.isFlymeLowerThan(8)
          || (DeviceHelper.isMIUI() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M)) {
        window.setFlags(
  ...
  /**
   * 已知系统类型时，设置状态栏黑色字体图标。 支持 4.4 以上版本 MIUI 和 Flyme，以及 6.0 以上版本的其他 Android
   *
   * @param activity 需要被处理的 Activity
  ...
  private static boolean isMIUICustomStatusBarLightModeImpl() {
    ...
    return DeviceHelper.isMIUIV5()
        || DeviceHelper.isMIUIV6()
        || DeviceHelper.isMIUIV7()
        || DeviceHelper.isMIUIV8();
  }
