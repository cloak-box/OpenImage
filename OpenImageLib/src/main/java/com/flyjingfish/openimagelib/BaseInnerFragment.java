...
public class BaseInnerFragment extends Fragment {
  ...
  private final List<ActivityResultCallback<Map<String, Boolean>>> activityResultCallbacks =
      new ArrayList<>();
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    launcher =
        registerForActivityResult(
            ...
            result -> {
              Iterator<ActivityResultCallback<Map<String, Boolean>>> iterator =
                  activityResultCallbacks.iterator();
              while (iterator.hasNext()) {
                ActivityResultCallback<Map<String, Boolean>> activityResultCallback =
                    iterator.next();
  ...
  private void addActivityResultCallback(
      ActivityResultCallback<Map<String, Boolean>> activityResultCallback) {
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    basePhotosViewModel.onAddItemListenerLiveData.observe(
        getViewLifecycleOwner(),
        s -> {
          OnItemClickListener onItemClickListener =
              ImageLoadUtils.getInstance().getOnItemClickListener(s);
          if (onItemClickListener != null) {
            onItemClickListeners.add(onItemClickListener);
          }
        });

    basePhotosViewModel.onAddItemLongListenerLiveData.observe(
        getViewLifecycleOwner(),
        s -> {
          OnItemLongClickListener onItemLongClickListener =
              ImageLoadUtils.getInstance().getOnItemLongClickListener(s);
          if (onItemLongClickListener != null) {
            onItemLongClickListeners.add(onItemLongClickListener);
          }
        });

    basePhotosViewModel.onRemoveItemListenerLiveData.observe(
        getViewLifecycleOwner(),
        s -> {
          OnItemClickListener onItemClickListener =
              ImageLoadUtils.getInstance().getOnItemClickListener(s);
    ...
    basePhotosViewModel.onRemoveItemLongListenerLiveData.observe(
        getViewLifecycleOwner(),
        s -> {
          OnItemLongClickListener onItemLongClickListener =
              ImageLoadUtils.getInstance().getOnItemLongClickListener(s);
    ...
    basePhotosViewModel.onTouchCloseLiveData.observe(
        getViewLifecycleOwner(),
        aFloat -> {
    ...
    basePhotosViewModel.onTouchScaleLiveData.observe(
        getViewLifecycleOwner(),
        aFloat -> {
          if (getViewLifecycleOwner().getLifecycle().getCurrentState() == Lifecycle.State.RESUMED) {
            onTouchScale(aFloat);
      ...
      if (onItemLongClickListener != null) {
        ImageLoadUtils.getInstance()
            .setOnItemLongClickListener(
                onItemLongClickListener.toString(), onItemLongClickListener);
        basePhotosViewModel.onAddItemLongListenerLiveData.setValue(
            onItemLongClickListener.toString());
  ...
  protected void addOnItemClickListener(OnItemClickListener onItemClickListener) {
    try {
      if (onItemClickListener != null) {
        ImageLoadUtils.getInstance()
            .setOnItemClickListener(onItemClickListener.toString(), onItemClickListener);
  ...
  protected void removeOnItemLongClickListener(OnItemLongClickListener onItemLongClickListener) {
    try {
      if (onItemLongClickListener != null) {
        basePhotosViewModel.onRemoveItemLongListenerLiveData.setValue(
            onItemLongClickListener.toString());
      }
    } catch (Exception e) {
      hintRuntimeException();
  ...
  protected void addOnSelectMediaListener(OnSelectMediaListener onSelectMediaListener) {
    try {
      if (onSelectMediaListener != null) {
        ImageLoadUtils.getInstance()
            .setOnSelectMediaListener(onSelectMediaListener.toString(), onSelectMediaListener);
        basePhotosViewModel.onAddOnSelectMediaListenerLiveData.setValue(
            onSelectMediaListener.toString());
      }
    } catch (Exception e) {
      hintRuntimeException();
    }
  ...
  protected void removeOnSelectMediaListener(OnSelectMediaListener onSelectMediaListener) {
    try {
      if (onSelectMediaListener != null) {
        basePhotosViewModel.onRemoveOnSelectMediaListenerLiveData.setValue(
            onSelectMediaListener.toString());
      }
    } catch (Exception e) {
      hintRuntimeException();
  ...
  protected void checkPermissionAndDownload(
      ...
      @Nullable String requestWriteExternalStoragePermissionsFail) {
  ...
  protected void downloadMedia(
      OpenImageUrl openImageUrl, OnDownloadMediaListener onDownloadMediaListener) {
    DownloadMediaHelper downloadMediaHelper =
        OpenImageConfig.getInstance().getDownloadMediaHelper();
    if (downloadMediaHelper == null) {
      if (ImageLoadUtils.getInstance().isApkInDebug()) {
  ...
  public void choseItem(OpenImageDetail openImageDetail, int position) {
    try {
      basePhotosViewModel.choseItemLiveData.setValue(new Pair<>(openImageDetail, position));
    } catch (Exception e) {
      hintRuntimeException();
    }
  }
