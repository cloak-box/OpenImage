...
/** 视频帮助类，更加节省资源 Created by guoshuyu on 2018/1/15. */
public class OpenImageGSYVideoHelper {
  ...
  public OpenImageGSYVideoHelper(Context context, StandardGSYVideoPlayer player) {
    ...
    this.mWindowViewContainer =
        (ViewGroup) (CommonUtil.scanForActivity(context)).findViewById(Window.ID_ANDROID_CONTENT);
    // 设置旋转
    mOrientationUtils =
        new OrientationUtils((Activity) mContext, mGsyVideoPlayer, mOrientationOption);
  }
  ...
  private void resolveToFull() {
    ...
    if (mOrientationUtils.getIsLand() == 0
        ...
        && mGsyVideoPlayer.isVerticalFullByVideoSize()) {
      return;
    }

    mSystemUiVisibility = ((Activity) mContext).getWindow().getDecorView().getSystemUiVisibility();
    CommonUtil.hideSupportActionBar(
        mContext, mVideoOptionBuilder.isHideActionBar(), mVideoOptionBuilder.isHideStatusBar());
          ...
          public void run() {
            ...
            if (mVideoOptionBuilder.getVideoAllCallBack() != null) {
              ...
              mVideoOptionBuilder
                  .getVideoAllCallBack()
                  .onQuitFullscreen(
                      mVideoOptionBuilder.getUrl(),
                      mVideoOptionBuilder.getVideoTitle(),
                      mGsyVideoPlayer);
            }
            if (mVideoOptionBuilder.isHideKey()) {
              showNavKey(mContext, mSystemUiVisibility);
            }
            CommonUtil.showSupportActionBar(
                mContext,
                mVideoOptionBuilder.isHideActionBar(),
  ...
  private void resolveChangeFirstLogic(int time) {
    ...
    if (mVideoOptionBuilder.getVideoAllCallBack() != null) {
      ...
      mVideoOptionBuilder
          .getVideoAllCallBack()
          .onEnterFullscreen(
              mVideoOptionBuilder.getUrl(), mVideoOptionBuilder.getVideoTitle(), mGsyVideoPlayer);
    }
  }
  ...
  public void addVideoPlayer(
      final int position, View imgView, String tag, ViewGroup container, View playBtn) {
  ...
  /**
   * 设置全屏显示的viewGroup 如果不设置即使用默认的 mWindowViewContainer
   *
